...
def test_projection_axes(axis):
    ...
    # a slice, oriented orthogonally to the direction of projection
    if axis == 0:
        gt = cp.zeros((1, 10, 100), dtype=cp.float32)
        gt[..., 90:100] = 1.0
        scan = [rotate(geom_t0, yaw=0.0)]
    elif axis == 1:
        gt = cp.zeros((10, 1, 100), dtype=cp.float32)
        gt[..., 90:100] = 1.0
        scan = [rotate(geom_t0, yaw=np.pi / 2)]
    elif axis == 2:
        gt = cp.zeros((100, 10, 1), dtype=cp.float32)
        gt[90:100] = 1.0
        ...
        scan = [rotate(geom_t0, pitch=np.pi / 2)]
    else:
        raise ValueError
    # gt = cp.ones((100, 100, 10), dtype=cp.float32)
    ...
    np.testing.assert_equal(np.sum(out), 64)
...
def test_invariant_under_voxel_size(nr_voxels):
    ...
    ----------
    nr_voxels : int
        The number of voxels to discretize the volume in one dimension.
    """
...
def test_volume_axes_choices(axes: tuple):
    ...
    gt = cp.ascontiguousarray(cp.transpose(gt, axes))
