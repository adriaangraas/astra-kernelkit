    ...
    def projection_geometry(self) -> list[ProjectionGeometry]:
        try:
            ...
            raise ProjectionGeometryNotSetError(
                ...
                f"Please set the projection geometry using `projection_geometry`.")
    ...
    def volume_geometry(self):
        try:
            ...
            raise VolumeGeometryNotSetError(
                ...
                f"Please set the volume geometry using `volume_geometry`.")
...
class ForwardProjector(Projector):
    ...
    def __init__(self, kernel: RayDrivenConeFP = None,
    ...
    def volume(self) -> cp.ndarray:
        ...
        try:
            ...
            raise VolumeNotSetError("Volume not set. Use the `volume` setter "
    ...
    def projections(self, value: cp.ndarray | list[cp.ndarray]):
        ...
        if not hasattr(self, '_proj_ptrs'):
            ...
            assert len(proj_ptrs) == len(self._proj_ptrs), (
...
class BackProjector(Projector):
    ...
    def __init__(self, kernel: VoxelDrivenConeBP = None,
                 texture_type: str = 'layered', **kernel_kwargs):
        ...
        if kernel is not None and len(kernel_kwargs) > 0:
            raise ValueError(
                f"If `kernel` is given, other keyword arguments cannot "
                f"be passed to the kernel constructor. You passed "
                f"`{kernel_kwargs}`.")
        super().__init__(K)
    ...
    def volume(self) -> cp.ndarray:
        ...
        try:
            ...
            raise VolumeNotSetError("No volume set. Please set the volume "
    ...
    def projections(self, value: Any):
        ...
        if self._texture_type == 'pitch2d':
            assert [ispitched(p) for p in value], (
                "Projections must be pitched, use "
                "`kernelkit.as_pitched(..., xp=cp)`.")
        else:
            if self._texture_type == 'layered':
                if self._K.projection_axes[0] != 0:
                    raise ValueError("Can only use layered texture when "
        ...
        if not hasattr(self, '_textures'):
            if self._texture_type in ('pitch2d', 'array2d'):
                ...
                self._texture_2d_objects = [
                    copy_to_texture(cp.asarray(p), self._texture_type)
                    for p in value]
                ...
                txt = copy_to_texture(value, 'array',
                                      layered=use_layered)
            else:
                ...
                for i, p in enumerate(value):
                    with cp.cuda.Stream(non_blocking=True):
                        if isinstance(p, np.ndarray):
                            ...
                            addr = p.data.ptr
                        (self._texture_2d_objects[i]
                         .ResDesc.arr.data
                         .copy_from_async(addr, p.nbytes))
    ...
    def __call__(self, additive: bool = False):
        ...
        if self._K.volume_axes == (2, 1, 0):
            ...
            self._vol[...] = cp.reshape(
                self._vol, tuple(reversed(self._vol.shape))).T
        else:
            raise NotImplementedError(
